...
public class MockTest : MonoBehaviour
{
    ...
    public Sprite mockIcon1;
    public Sprite mockIcon2;

    private int testActiveSlot = 0;
    ...
    void Update()
    {
        ...
        // 1, 2, 3 tuþlarýna basarak gezinme (Hala çalýþsýn istersen)
        if (Input.GetKeyDown(KeyCode.Alpha1)) testActiveSlot = 0;
        if (Input.GetKeyDown(KeyCode.Alpha2)) testActiveSlot = 1;
        if (Input.GetKeyDown(KeyCode.Alpha3)) testActiveSlot = 2;

        // --- FARE TEKERLEÐÝ (SCROLL) ÝLE GEZÝNME ---
        float scroll = Input.mouseScrollDelta.y;

        if (scroll > 0) // Tekerleði ileri ittiysek (Yukarý)
        {
            testActiveSlot--;
            if (testActiveSlot < 0) testActiveSlot = 2; // 0'dan geriye giderse 2. slota (sona) atla
        }
        else if (scroll < 0) // Tekerleði geri çektiysek (Aþaðý)
        {
            testActiveSlot++;
            if (testActiveSlot > 2) testActiveSlot = 0; // 2'den ileri giderse 0. slota (baþa) atla
        }

        // Eðer fare tekerleði oynatýldýysa veya tuþlara basýldýysa UI'ý güncelle
        if (scroll != 0 || Input.GetKeyDown(KeyCode.Alpha1) || Input.GetKeyDown(KeyCode.Alpha2) || Input.GetKeyDown(KeyCode.Alpha3))
        {
            // Sahte bir envanter dizisi oluþtur ve UI'a yolla
            Sprite[] testItems = new Sprite[3] { mockIcon1, mockIcon2, null };
            FindObjectOfType<InventoryUIController>().UpdateInventory(testItems, testActiveSlot, 14.0f);
        }
    }
}
